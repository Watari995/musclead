...
サーバー側で `cookies()` から `refresh token` を読み、 Backend を直接 fetch して
ページを SSR する。 page.tsx は async Server Component に書き換える。

**Pros**
- 初回表示が速くなる(JS bundle 待ちなし)
- Backend トークン管理がサーバー側で完結 → XSS で access token を盗まれない
- TanStack Query を一部不要にできる(Server Component 内で fetch すれば良い)

**Cons**
- **Backend 認証フローを Cookie ベースに変更必須**
  - 現状: `Authorization: Bearer <access_token>` (JS 管理)
  - 移行後: Backend が `HttpOnly Cookie` 経由でも access token を受けるか、
...
3. **Backend 認証フローを Cookie ベースに変更する別の動機が出る** → 案 A
...
- RSC 移行の **判断材料を貯める時間** が確保できる
  (案 A をやるなら Backend 認証設計から見直すのが筋)
...
1. **Backend の認証フローを Cookie 対応にする ADR**
...
3. **Client / Server Component の分割境界を文書化**
   - 「データ取得は Backend」 「インタラクションは Client」 が基本
   - features/ 配下でどう構造化するか(例: `features/training/api/server.ts`
     に Backend-side fetchers を分離)
