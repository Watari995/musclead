...
| 層 | サービス | 月額 (初年度) | 月額 (翌年) |
|---|---|---|---|
| Backend container | ECS Fargate Spot 0.25 vCPU / 0.5 GB | $3 | $3 |
...
- `api.musclead.com` → Backend Fargate task
...
3. `terraform/ecr/`: ECR Private リポジトリ (Backend / FE 用)
