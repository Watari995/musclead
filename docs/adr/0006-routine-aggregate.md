...
3. **expand を BE で集約横断ユースケースとして実装するか、 FE が orchestrate するか**
...
「ルーティンから training を作成」 専用エンドポイントは BE に作らない。 FE が:
...
c. **expand を FE 主導にする**: 「ルーティンを使う = 種目順だけコピー」 という単純さは
   FE で十分扱える。 BE に集約横断ユースケースを増やすほどの抽象化は不要。
...
C. **「ルーティンから training を作成」 専用エンドポイント (POST /routines/{id}/start) を作る**
   - 棄却: BE が集約横断 usecase(Routine 読 + Training 書)を持つことになり、 集約境界が
